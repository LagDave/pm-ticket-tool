...
export function TriageBranch({ sessionId, onRouted }: TriageBranchProps) {
  ...
  // Classifying, or the auto-run hasn't produced a result yet.
  if (triage.isPending || (!outcome && !triage.isError)) {
    return (
      <section className="step-panel">
        <h2 className="step-heading">Sizing up your request</h2>
        <p className="field-hint">
          Deciding whether this needs the full interview or can go straight to a
          draft ticket…
        </p>
      </section>
    );
  ...
  return (
    <section className="step-panel">
      ...
      <p className="field-hint">
        {isSimple
          ? "We can draft a ticket from your request directly — no interview needed. You can still run the full interview if you'd rather."
          : "There are open product decisions here, so we'll walk through a short interview before drafting. You can skip straight to a draft if you prefer."}
      </p>
        ...
        {isSimple ? (
          <>
            ...
            <button
              ...
              onClick={() => onRouted("interview")}
            >
              Run the full interview instead
            </button>
          </>
        ) : (
          <>
